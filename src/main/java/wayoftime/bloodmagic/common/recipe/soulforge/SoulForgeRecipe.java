...
public class SoulForgeRecipe implements Recipe<SoulForgeInput> {
    ...
    public final Optional<DemonWillType> willType;
    public SoulForgeRecipe(double minWill, double usedWill, List<Ingredient> ingredients, ItemStack resultItem, Optional<DemonWillType> willType) {
    ...
    @Override
    public boolean matches(SoulForgeInput input, Level level) {
        if (input.size() != ingredients.size()) {
            return false;
        }
        DemonWillType will = input.getGem().getOrDefault(BMDataComponents.DEMON_WILL_TYPE, DemonWillType.DEFAULT);
    ...
    @Override
    public ItemStack assemble(SoulForgeInput input, HolderLookup.Provider registries) {
    ...
    public Optional<DemonWillType> getWillType() {
        return willType;
    }
}
